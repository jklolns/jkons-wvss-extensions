"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.validWorkspaceCMD = void 0;
const vscode = require("vscode");
let win = vscode.window;
// Checks if a workspace folder is there and sends a corresponding message
function validWorkspaceCMD(commandName, commandCallback, successMessage) {
    let disposable = vscode.commands.registerCommand(commandName, () => {
        if (vscode.workspace.workspaceFolders) {
            commandCallback();
            if (successMessage) {
                win.showInformationMessage(
                // Success message
                successMessage);
            }
            return true;
        }
        else {
            // Error message
            win.showErrorMessage("No workspace found. \n Please open a folder or a workspace and try again.");
            return false;
        }
    });
    return disposable;
}
exports.validWorkspaceCMD = validWorkspaceCMD;
//# sourceMappingURL=helper.js.map